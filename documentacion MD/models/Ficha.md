...
| `6` | `    private $tabla = "fichas";` | Propiedad `private` `$tabla` inicializado en `"fichas"` para el estado interno de la clase. |
...
| `22` | `             LEFT JOIN voceros   v ON v.id_ficha    = f.id_ficha AND v.acti...` | Instrucción de ejecución en el contexto del script: `LEFT JOIN voceros   v ON v.id_ficha    = f.id_ficha AND v.activo = 1`. |
| `23` | `             LEFT JOIN aprendices a ON a.id_ficha   = f.id_ficha AND a.acti...` | Instrucción de ejecución en el contexto del script: `LEFT JOIN aprendices a ON a.id_ficha   = f.id_ficha AND a.activo = 1`. |
...
| `38` | `        );` | Instrucción de ejecución en el contexto del script: `);`. |
| `39` | `        $stmt->execute([':id' => $id]);` | Ejecuta la sentencia SQL preparada vinculando los datos correspondientes: `$stmt->execute([':id' => $id]);`. |
| `40` | `        return $stmt->fetch(PDO::FETCH_ASSOC);` | Retorna el valor resultante de la expresión y culmina la ejecución de la función actual: `return $stmt->fetch(PDO::FETCH_ASSOC);`. |
...
| `49` | `        );` | Instrucción de ejecución en el contexto del script: `);`. |
| `50` | `        $stmt->execute([':id' => $idFicha]);` | Ejecuta la sentencia SQL preparada vinculando los datos correspondientes: `$stmt->execute([':id' => $idFicha]);`. |
| `51` | `        return $stmt->fetchAll(PDO::FETCH_ASSOC);` | Retorna el valor resultante de la expresión y culmina la ejecución de la función actual: `return $stmt->fetchAll(PDO::FETCH_ASSOC);`. |
...
| `54` | `    public function existeDuplicado(string $numero, int $idPrograma, ?int $...` | Declaración de método o función con su firma y parámetros: `public function existeDuplicado(string $numero, int $idPrograma, ?int $excluirId = null): bool`. |
...
| `57` | `                WHERE numero_ficha = :numero AND id_programa = :prog AND ac...` | Instrucción de ejecución en el contexto del script: `WHERE numero_ficha = :numero AND id_programa = :prog AND activo = 1";`. |
...
| `62` | `        $stmt->execute($params);` | Ejecuta la sentencia SQL preparada vinculando los datos correspondientes: `$stmt->execute($params);`. |
| `63` | `        return $stmt->rowCount() > 0;` | Retorna el valor resultante de la expresión y culmina la ejecución de la función actual: `return $stmt->rowCount() > 0;`. |
...
| `68` | `        try {` | Inicia bloque de captura de excepciones `try` para ejecución segura de operaciones críticas. |
...
| `70` | `                "INSERT INTO {$this->tabla} (id_programa, numero_ficha, jor...` | Instrucción de ejecución en el contexto del script: `"INSERT INTO {$this->tabla} (id_programa, numero_ficha, jornada, num_aprendices)`. |
| `71` | `                 VALUES (:id_programa, :numero_ficha, :jornada, :num_aprend...` | Instrucción de ejecución en el contexto del script: `VALUES (:id_programa, :numero_ficha, :jornada, :num_aprendices)"`. |
| `72` | `            );` | Instrucción de ejecución en el contexto del script: `);`. |
| `73` | `            $stmt->execute([` | Ejecuta la sentencia SQL preparada vinculando los datos correspondientes: `$stmt->execute([`. |
...
| `79` | `            return (int) $this->conn->lastInsertId();` | Retorna el valor resultante de la expresión y culmina la ejecución de la función actual: `return (int) $this->conn->lastInsertId();`. |
| `80` | `        } catch (Exception $e) {` | Captura y manejo de excepciones en caso de fallo durante el bloque protegido: `} catch (Exception $e) {`. |
| `81` | `            return false;` | Retorna el valor resultante de la expresión y culmina la ejecución de la función actual: `return false;`. |
...
| `92` | `        );` | Instrucción de ejecución en el contexto del script: `);`. |
| `93` | `        return $stmt->execute([` | Retorna el valor resultante de la expresión y culmina la ejecución de la función actual: `return $stmt->execute([`. |
...
| `107` | `        );` | Instrucción de ejecución en el contexto del script: `);`. |
| `108` | `        $stmt->execute([':id' => $id]);` | Ejecuta la sentencia SQL preparada vinculando los datos correspondientes: `$stmt->execute([':id' => $id]);`. |
| `109` | `        return $stmt->rowCount() > 0;` | Retorna el valor resultante de la expresión y culmina la ejecución de la función actual: `return $stmt->rowCount() > 0;`. |
...
| `117` | `        return $stmt->execute([':id' => $id]);` | Retorna el valor resultante de la expresión y culmina la ejecución de la función actual: `return $stmt->execute([':id' => $id]);`. |
