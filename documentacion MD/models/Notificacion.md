...
| `6` | `    private $tabla = "notificaciones";` | Propiedad `private` `$tabla` inicializado en `"notificaciones"` para el estado interno de la clase. |
...
| `16` | `            "INSERT INTO {$this->tabla} (id_usuario, id_asignacion, tipo, t...` | Instrucción de ejecución en el contexto del script: `"INSERT INTO {$this->tabla} (id_usuario, id_asignacion, tipo, titulo, mensaje)`. |
...
| `19` | `        return $stmt->execute([` | Retorna el valor resultante de la expresión y culmina la ejecución de la función actual: `return $stmt->execute([`. |
...
| `28` | `    public function obtenerPorUsuario(int $idUsuario, bool $soloNoLeidas = ...` | Declaración de método o función con su firma y parámetros: `public function obtenerPorUsuario(int $idUsuario, bool $soloNoLeidas = false): array`. |
...
| `35` | `        $stmt->execute([':id' => $idUsuario]);` | Ejecuta la sentencia SQL preparada vinculando los datos correspondientes: `$stmt->execute([':id' => $idUsuario]);`. |
| `36` | `        return $stmt->fetchAll(PDO::FETCH_ASSOC);` | Retorna el valor resultante de la expresión y culmina la ejecución de la función actual: `return $stmt->fetchAll(PDO::FETCH_ASSOC);`. |
...
| `45` | `        $stmt->execute([':id' => $idUsuario]);` | Ejecuta la sentencia SQL preparada vinculando los datos correspondientes: `$stmt->execute([':id' => $idUsuario]);`. |
| `46` | `        return (int) $stmt->fetchColumn();` | Retorna el valor resultante de la expresión y culmina la ejecución de la función actual: `return (int) $stmt->fetchColumn();`. |
...
| `53` | `        );` | Instrucción de ejecución en el contexto del script: `);`. |
| `54` | `        return $stmt->execute([':id' => $idNotificacion]);` | Retorna el valor resultante de la expresión y culmina la ejecución de la función actual: `return $stmt->execute([':id' => $idNotificacion]);`. |
...
| `61` | `        );` | Instrucción de ejecución en el contexto del script: `);`. |
| `62` | `        return $stmt->execute([':id' => $idUsuario]);` | Retorna el valor resultante de la expresión y culmina la ejecución de la función actual: `return $stmt->execute([':id' => $idUsuario]);`. |
...
| `65` | `    public function obtenerHistorialIncumplimientos(int $idUsuario, array $...` | Declaración de método o función con su firma y parámetros: `public function obtenerHistorialIncumplimientos(int $idUsuario, array $filtros = []): array`. |
...
| `90` | `        );` | Instrucción de ejecución en el contexto del script: `);`. |
| `91` | `        $stmt->execute($params);` | Ejecuta la sentencia SQL preparada vinculando los datos correspondientes: `$stmt->execute($params);`. |
| `92` | `        return $stmt->fetchAll(PDO::FETCH_ASSOC);` | Retorna el valor resultante de la expresión y culmina la ejecución de la función actual: `return $stmt->fetchAll(PDO::FETCH_ASSOC);`. |
