...
| `6` | `private $tabla = "programas";` | Definición de propiedad de clase para el estado interno del componente: `private $tabla = "programas";`. |
...
| `18` | `LEFT JOIN fichas f ON f.id_programa = p.id_programa AND f.activo = 1` | Instrucción de ejecución en el contexto del script: `LEFT JOIN fichas f ON f.id_programa = p.id_programa AND f.activo = 1`. |
...
| `21` | ``);`` | Cierre de estructura de arreglo o invocación de función. |
| `22` | `return $stmt->fetchAll(PDO::FETCH_ASSOC);` | Obtiene todos los registros coincidentes de la consulta en un arreglo asociativo. |
...
| `29` | ``);`` | Cierre de estructura de arreglo o invocación de función. |
| `30` | `$stmt->execute([':id' => $id]);` | Ejecuta la sentencia SQL preparada vinculando los parámetros correspondientes: `$stmt->execute([':id' => $id]);`. |
| `31` | `return $stmt->fetch(PDO::FETCH_ASSOC);` | Recupera una única fila o registro resultante de la consulta. |
...
| `36` | `try {` | Instrucción de ejecución en el contexto del script: `try {`. |
...
| `40` | ``);`` | Cierre de estructura de arreglo o invocación de función. |
| `41` | `$stmt->execute([` | Ejecuta la sentencia SQL preparada vinculando los parámetros correspondientes: `$stmt->execute([`. |
...
| `46` | `return (int) $this->conn->lastInsertId();` | Instrucción de retorno que finaliza la ejecución entregando el resultado: `return (int) $this->conn->lastInsertId();`. |
| `47` | `} catch (Exception $e) {` | Instrucción de ejecución en el contexto del script: `} catch (Exception $e) {`. |
| `48` | `return false;` | Instrucción de retorno que finaliza la ejecución entregando el resultado: `return false;`. |
...
| `59` | `return $stmt->execute([` | Ejecuta la sentencia SQL preparada vinculando los parámetros correspondientes: `return $stmt->execute([`. |
...
| `70` | `"SELECT id_ficha FROM fichas WHERE id_programa = :id AND activo = 1 LIMI...` | Instrucción de ejecución en el contexto del script: `"SELECT id_ficha FROM fichas WHERE id_programa = :id AND activo = 1 LIMI...`. |
| `71` | ``);`` | Cierre de estructura de arreglo o invocación de función. |
| `72` | `$stmt->execute([':id' => $id]);` | Ejecuta la sentencia SQL preparada vinculando los parámetros correspondientes: `$stmt->execute([':id' => $id]);`. |
| `73` | `return $stmt->rowCount() > 0;` | Instrucción de retorno que finaliza la ejecución entregando el resultado: `return $stmt->rowCount() > 0;`. |
...
| `81` | `return $stmt->execute([':id' => $id]);` | Ejecuta la sentencia SQL preparada vinculando los parámetros correspondientes: `return $stmt->execute([':id' => $id]);`. |
...
| `91` | ``);`` | Cierre de estructura de arreglo o invocación de función. |
| `92` | `$stmt->execute([':t' => $like, ':t2' => $like]);` | Ejecuta la sentencia SQL preparada vinculando los parámetros correspondientes: `$stmt->execute([':t' => $like, ':t2' => $like]);`. |
| `93` | `return $stmt->fetchAll(PDO::FETCH_ASSOC);` | Obtiene todos los registros coincidentes de la consulta en un arreglo asociativo. |
