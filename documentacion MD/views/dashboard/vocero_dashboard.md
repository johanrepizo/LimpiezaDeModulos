...
| `13` | `$idUsuario = (int)$_SESSION['usuario']['id_usuario'];` | Accede o almacena información de identidad del usuario en la sesión activa: `$idUsuario = (int)$_SESSION['usuario']['id_usuario'];`. |
| `14` | *(Línea en blanco)* | Línea en blanco para organización visual y legibilidad. |
| `15` | `// Obtener datos del vocero` | Comentario de línea explicativo: `Obtener datos del vocero`. |
...
| `22` | `);` | Instrucción de ejecución en el contexto del script: `);`. |
| `23` | `$stmtV->execute([':id' => $idUsuario]);` | Ejecuta la sentencia SQL preparada vinculando los datos correspondientes: `$stmtV->execute([':id' => $idUsuario]);`. |
...
| `27` | `    // Usuario con rol vocero pero sin perfil de vocero creado todavía` | Comentario de línea explicativo: `Usuario con rol vocero pero sin perfil de vocero creado todavía`. |
| `28` | `    // NO destruimos la sesión — mostramos mensaje de espera` | Comentario de línea explicativo: `NO destruimos la sesión — mostramos mensaje de espera`. |
...
| `31` | `    <div class="d-flex align-items-center justify-content-center" style="mi...` | Contenedor visual estructurado con Bootstrap/CSS: `<div class="d-flex align-items-center justify-content-center" style="min-height:60vh;">`. |
...
| `36` | `                Tu cuenta fue creada correctamente, pero el administrador a...` | Instrucción de ejecución en el contexto del script: `Tu cuenta fue creada correctamente, pero el administrador aún no ha`. |
...
| `40` | `            <a href="../../controllers/AuthController.php?accion=logout"` | Enlace hipertexto de navegación o acción: `<a href="../../controllers/AuthController.php?accion=logout"`. |
...
| `47` | `    exit;` | Detiene inmediatamente la ejecución del script PHP en el servidor. |
...
| `52` | `// Verificar si hoy hay turno de limpieza para esta ficha` | Comentario de línea explicativo: `Verificar si hoy hay turno de limpieza para esta ficha`. |
...
| `55` | `$turnoAlerta   = $idVocero ? $modelTurnoHoy->turnoActivoHoy($vocero['id_fic...` | Instrucción de ejecución en el contexto del script: `$turnoAlerta   = $idVocero ? $modelTurnoHoy->turnoActivoHoy($vocero['id_ficha']) : false;`. |
| `56` | *(Línea en blanco)* | Línea en blanco para organización visual y legibilidad. |
| `57` | `// Stats` | Comentario de línea explicativo: `Stats`. |
| `58` | `$totalGrupos = $db->prepare("SELECT COUNT(*) FROM grupos WHERE id_vocero = ...` | Prepara de forma segura una sentencia SQL parametrizada contra inyección SQL: `$totalGrupos = $db->prepare("SELECT COUNT(*) FROM grupos WHERE id_vocero = :id");`. |
| `59` | `$totalGrupos->execute([':id' => $idVocero]);` | Ejecuta la sentencia SQL preparada vinculando los datos correspondientes: `$totalGrupos->execute([':id' => $idVocero]);`. |
| `60` | `$totalGrupos = $totalGrupos->fetchColumn();` | Obtiene el valor de una columna única de la primera fila resultante. |
| `61` | *(Línea en blanco)* | Línea en blanco para organización visual y legibilidad. |
| `62` | `$totalEvidencias = $db->prepare("SELECT COUNT(*) FROM evidencias WHERE id_v...` | Prepara de forma segura una sentencia SQL parametrizada contra inyección SQL: `$totalEvidencias = $db->prepare("SELECT COUNT(*) FROM evidencias WHERE id_vocero = :id");`. |
| `63` | `$totalEvidencias->execute([':id' => $idVocero]);` | Ejecuta la sentencia SQL preparada vinculando los datos correspondientes: `$totalEvidencias->execute([':id' => $idVocero]);`. |
| `64` | `$totalEvidencias = $totalEvidencias->fetchColumn();` | Obtiene el valor de una columna única de la primera fila resultante. |
| `65` | *(Línea en blanco)* | Línea en blanco para organización visual y legibilidad. |
| `66` | `$totalAprendices = $db->prepare("SELECT COUNT(*) FROM aprendices WHERE id_f...` | Prepara de forma segura una sentencia SQL parametrizada contra inyección SQL: `$totalAprendices = $db->prepare("SELECT COUNT(*) FROM aprendices WHERE id_ficha = :id AND activo=1");`. |
| `67` | `$totalAprendices->execute([':id' => $vocero['id_ficha']]);` | Ejecuta la sentencia SQL preparada vinculando los datos correspondientes: `$totalAprendices->execute([':id' => $vocero['id_ficha']]);`. |
| `68` | `$totalAprendices = $totalAprendices->fetchColumn();` | Obtiene el valor de una columna única de la primera fila resultante. |
| `69` | *(Línea en blanco)* | Línea en blanco para organización visual y legibilidad. |
| `70` | `// Asignaciones activas de mi ficha` | Comentario de línea explicativo: `Asignaciones activas de mi ficha`. |
...
| `82` | `$asignaciones->execute([':fic' => $vocero['id_ficha'], ':idv' => $idVocero]);` | Ejecuta la sentencia SQL preparada vinculando los datos correspondientes: `$asignaciones->execute([':fic' => $vocero['id_ficha'], ':idv' => $idVocero]);`. |
...
| `85` | `// Últimas evidencias` | Comentario de línea explicativo: `Últimas evidencias`. |
...
| `96` | `);` | Instrucción de ejecución en el contexto del script: `);`. |
| `97` | `$ultimasEv->execute([':id' => $idVocero]);` | Ejecuta la sentencia SQL preparada vinculando los datos correspondientes: `$ultimasEv->execute([':id' => $idVocero]);`. |
...
| `105` | `     style="background:#fffbeb; border-left:4px solid #f59e0b !important; b...` | Instrucción de ejecución en el contexto del script: `style="background:#fffbeb; border-left:4px solid #f59e0b !important; border-radius:10px;">`. |
| `106` | `    <i class="fas fa-triangle-exclamation mt-1" style="color:#f59e0b; font-...` | Instrucción de ejecución en el contexto del script: `<i class="fas fa-triangle-exclamation mt-1" style="color:#f59e0b; font-size:1.2rem; flex-shrink:0;"></i>`. |
| `107` | `    <div>` | Contenedor visual estructurado con Bootstrap/CSS: `<div>`. |
| `108` | `        <div class="fw-bold" style="color:#92400e;">¡Hoy es día de limpieza...` | Contenedor visual estructurado con Bootstrap/CSS: `<div class="fw-bold" style="color:#92400e;">¡Hoy es día de limpieza!</div>`. |
| `109` | `        <div class="small text-muted">` | Contenedor visual estructurado con Bootstrap/CSS: `<div class="small text-muted">`. |
| `110` | `            Hoy le corresponde limpiar el módulo <strong><?= htmlspecialcha...` | Instrucción de ejecución en el contexto del script: `Hoy le corresponde limpiar el módulo <strong><?= htmlspecialchars($turnoAlerta['nombre_modulo']) ?></strong>.`. |
| `111` | `            Recuerda subir la evidencia fotográfica antes de las <strong>11...` | Instrucción de ejecución en el contexto del script: `Recuerda subir la evidencia fotográfica antes de las <strong>11:59 PM</strong>.`. |
| `112` | `            <?php if ($turnoAlerta['nombre_grupo']): ?>` | Instrucción de ejecución en el contexto del script: `<?php if ($turnoAlerta['nombre_grupo']): ?>`. |
| `113` | `                Grupo responsable: <strong><?= htmlspecialchars($turnoAlert...` | Instrucción de ejecución en el contexto del script: `Grupo responsable: <strong><?= htmlspecialchars($turnoAlerta['nombre_grupo']) ?></strong>.`. |
| `114` | `            <?php endif; ?>` | Instrucción de ejecución en el contexto del script: `<?php endif; ?>`. |
| `115` | `        </div>` | Cierre de contenedor visual `<div>`. |
| `116` | `        <a href="vocero_subir_evidencia.php" class="btn btn-sm mt-2 fw-semi...` | Enlace hipertexto de navegación o acción: `<a href="vocero_subir_evidencia.php" class="btn btn-sm mt-2 fw-semibold"`. |
| `117` | `           style="background:#f59e0b; color:#fff; border:none; border-radiu...` | Instrucción de ejecución en el contexto del script: `style="background:#f59e0b; color:#fff; border:none; border-radius:7px;">`. |
...
| `122` | `<?php endif; ?>` | Instrucción de ejecución en el contexto del script: `<?php endif; ?>`. |
...
| `131` | `            Ficha <strong><?= htmlspecialchars($vocero['numero_ficha']) ?><...` | Instrucción de ejecución en el contexto del script: `Ficha <strong><?= htmlspecialchars($vocero['numero_ficha']) ?></strong>`. |
...
| `142` | `                <div class="stat-icon" style="background:rgba(57,169,0,.12)...` | Contenedor visual estructurado con Bootstrap/CSS: `<div class="stat-icon" style="background:rgba(57,169,0,.12); color:#39a900;"><i class="fas fa-users"></i></div>`. |
...
| `153` | `                <div class="stat-icon" style="background:rgba(37,99,235,.1)...` | Contenedor visual estructurado con Bootstrap/CSS: `<div class="stat-icon" style="background:rgba(37,99,235,.1); color:#2563eb;"><i class="fas fa-people-group"></i></div>`. |
...
| `164` | `                <div class="stat-icon" style="background:rgba(234,179,8,.1)...` | Contenedor visual estructurado con Bootstrap/CSS: `<div class="stat-icon" style="background:rgba(234,179,8,.1); color:#d97706;"><i class="fas fa-images"></i></div>`. |
...
| `179` | `                <h6 class="fw-bold mb-0"><i class="fas fa-door-open text-su...` | Instrucción de ejecución en el contexto del script: `<h6 class="fw-bold mb-0"><i class="fas fa-door-open text-success me-2"></i>Mis Módulos Asignados</h6>`. |
...
| `184` | `                        <i class="fas fa-door-open fa-2x mb-2 opacity-25 d-...` | Instrucción de ejecución en el contexto del script: `<i class="fas fa-door-open fa-2x mb-2 opacity-25 d-block"></i>`. |
| `185` | `                        <span class="small">No tienes módulos asignados act...` | Instrucción de ejecución en el contexto del script: `<span class="small">No tienes módulos asignados actualmente.</span>`. |
...
| `190` | `                        $vencido   = strtotime($a['fecha_limite_evidencia']...` | Instrucción de ejecución en el contexto del script: `$vencido   = strtotime($a['fecha_limite_evidencia']) < time();`. |
...
| `194` | `                        <div class="d-flex justify-content-between align-it...` | Contenedor visual estructurado con Bootstrap/CSS: `<div class="d-flex justify-content-between align-items-start">`. |
| `195` | `                            <div>` | Contenedor visual estructurado con Bootstrap/CSS: `<div>`. |
| `196` | `                                <div class="fw-semibold small"><?= htmlspec...` | Contenedor visual estructurado con Bootstrap/CSS: `<div class="fw-semibold small"><?= htmlspecialchars($a['nombre_modulo']) ?></div>`. |
| `197` | `                                <div class="text-muted" style="font-size:.7...` | Contenedor visual estructurado con Bootstrap/CSS: `<div class="text-muted" style="font-size:.78rem;">`. |
| `198` | `                                    Límite: <?= date('d/m/Y H:i', strtotime...` | Instrucción de ejecución en el contexto del script: `Límite: <?= date('d/m/Y H:i', strtotime($a['fecha_limite_evidencia'])) ?>`. |
| `199` | `                                </div>` | Cierre de contenedor visual `<div>`. |
| `200` | `                            </div>` | Cierre de contenedor visual `<div>`. |
| `201` | `                            <div class="d-flex flex-column align-items-end ...` | Contenedor visual estructurado con Bootstrap/CSS: `<div class="d-flex flex-column align-items-end gap-1">`. |
| `202` | `                                <?php if ($cumple): ?>` | Instrucción de ejecución en el contexto del script: `<?php if ($cumple): ?>`. |
| `203` | `                                    <span class="badge bg-success">✓ Entreg...` | Instrucción de ejecución en el contexto del script: `<span class="badge bg-success">✓ Entregada</span>`. |
| `204` | `                                <?php elseif ($vencido): ?>` | Instrucción de ejecución en el contexto del script: `<?php elseif ($vencido): ?>`. |
| `205` | `                                    <span class="badge bg-danger">Vencido</...` | Instrucción de ejecución en el contexto del script: `<span class="badge bg-danger">Vencido</span>`. |
| `206` | `                                <?php else: ?>` | Instrucción de ejecución en el contexto del script: `<?php else: ?>`. |
| `207` | `                                    <span class="badge bg-warning text-dark...` | Instrucción de ejecución en el contexto del script: `<span class="badge bg-warning text-dark">Pendiente</span>`. |
| `208` | `                                <?php endif; ?>` | Instrucción de ejecución en el contexto del script: `<?php endif; ?>`. |
| `209` | `                                <?php if (!$cumple && !$vencido): ?>` | Instrucción de ejecución en el contexto del script: `<?php if (!$cumple && !$vencido): ?>`. |
| `210` | `                                    <a href="vocero_evidencias.php?asig=<?=...` | Enlace hipertexto de navegación o acción: `<a href="vocero_evidencias.php?asig=<?= $a['id_asignacion'] ?>"`. |
| `211` | `                                       class="btn btn-sm btn-success py-0 p...` | Instrucción de ejecución en el contexto del script: `class="btn btn-sm btn-success py-0 px-2" style="font-size:.75rem;">`. |
| `212` | `                                        <i class="fas fa-upload me-1"></i>S...` | Instrucción de ejecución en el contexto del script: `<i class="fas fa-upload me-1"></i>Subir evidencia`. |
| `213` | `                                    </a>` | Instrucción de ejecución en el contexto del script: `</a>`. |
| `214` | `                                <?php endif; ?>` | Instrucción de ejecución en el contexto del script: `<?php endif; ?>`. |
...
| `218` | `                    <?php endforeach; ?>` | Instrucción de ejecución en el contexto del script: `<?php endforeach; ?>`. |
| `219` | `                    </ul>` | Instrucción de ejecución en el contexto del script: `</ul>`. |
| `220` | `                <?php endif; ?>` | Instrucción de ejecución en el contexto del script: `<?php endif; ?>`. |
...
| `229` | `                <h6 class="fw-bold mb-0"><i class="fas fa-clock-rotate-left...` | Instrucción de ejecución en el contexto del script: `<h6 class="fw-bold mb-0"><i class="fas fa-clock-rotate-left text-success me-2"></i>Últimas Evidencias</h6>`. |
...
| `233` | `                    <p class="text-muted small text-center py-3">Aún no has...` | Instrucción de ejecución en el contexto del script: `<p class="text-muted small text-center py-3">Aún no has subido evidencias.</p>`. |
...
| `238` | `                        <div class="position-relative" style="border-radius...` | Contenedor visual estructurado con Bootstrap/CSS: `<div class="position-relative" style="border-radius:8px; overflow:hidden; border:1px solid #e5e7eb;">`. |
| `239` | `                            <img src="../../public/<?= htmlspecialchars($ev...` | Instrucción de ejecución en el contexto del script: `<img src="../../public/<?= htmlspecialchars($ev['ruta_archivo']) ?>"`. |
| `240` | `                                 alt="Evidencia"` | Instrucción de ejecución en el contexto del script: `alt="Evidencia"`. |
| `241` | `                                 style="width:100%; height:80px; object-fit...` | Instrucción de ejecución en el contexto del script: `style="width:100%; height:80px; object-fit:cover;">`. |
| `242` | `                            <div style="position:absolute; bottom:0; left:0...` | Contenedor visual estructurado con Bootstrap/CSS: `<div style="position:absolute; bottom:0; left:0; right:0; background:rgba(0,0,0,.6); color:#fff; font-size:.65rem; padding:3px 6px;">`. |
| `243` | `                                <?= htmlspecialchars($ev['modulo']) ?><br>` | Instrucción de ejecución en el contexto del script: `<?= htmlspecialchars($ev['modulo']) ?><br>`. |
| `244` | `                                <?= date('d/m/Y', strtotime($ev['fecha_subi...` | Instrucción de ejecución en el contexto del script: `<?= date('d/m/Y', strtotime($ev['fecha_subida'])) ?>`. |
| `245` | `                            </div>` | Cierre de contenedor visual `<div>`. |
| `246` | `                        </div>` | Cierre de contenedor visual `<div>`. |
| `247` | `                    </div>` | Cierre de contenedor visual `<div>`. |
| `248` | `                    <?php endforeach; ?>` | Instrucción de ejecución en el contexto del script: `<?php endforeach; ?>`. |
| `249` | `                    </div>` | Cierre de contenedor visual `<div>`. |
| `250` | `                    <a href="vocero_evidencias.php" class="btn btn-sm btn-o...` | Enlace hipertexto de navegación o acción: `<a href="vocero_evidencias.php" class="btn btn-sm btn-outline-success w-100 mt-3">Ver todas</a>`. |
| `251` | `                <?php endif; ?>` | Instrucción de ejecución en el contexto del script: `<?php endif; ?>`. |
...
| `260` | `document.addEventListener('DOMContentLoaded', function() {` | Instrucción de ejecución en el contexto del script: `document.addEventListener('DOMContentLoaded', function() {`. |
| `261` | `    Swal.fire({ icon:'<?= addslashes($alert['icon']) ?>', title:'<?= addsla...` | Instrucción de ejecución en el contexto del script: `Swal.fire({ icon:'<?= addslashes($alert['icon']) ?>', title:'<?= addslashes($alert['title']) ?>', text:'<?= addslashes($alert['text']) ?>', confirmButtonColor:'#39a900' });`. |
| `262` | `});` | Instrucción de ejecución en el contexto del script: `});`. |
| `263` | `</script>` | Bloque o etiqueta de vinculación para scripts JavaScript del lado del cliente. |
...
| `266` | `<?php require_once __DIR__ . '/../layouts/footer.php'; ?>` | Instrucción de ejecución en el contexto del script: `<?php require_once __DIR__ . '/../layouts/footer.php'; ?>`. |
