...
| `14` | `// Marcar todas como leídas al entrar` | Comentario explicativo en el código: `Marcar todas como leídas al entrar`. |
...
| `17` | `// Historial completo` | Comentario explicativo en el código: `Historial completo`. |
...
| `26` | `<h4 class="fw-bold mb-0"><i class="fas fa-bell text-success me-2"></i>Mi...` | Instrucción de ejecución en el contexto del script: `<h4 class="fw-bold mb-0"><i class="fas fa-bell text-success me-2"></i>Mi...`. |
| `27` | `<p class="text-muted small mb-0">Alertas e incumplimientos de tu ficha</p>` | Instrucción de ejecución en el contexto del script: `<p class="text-muted small mb-0">Alertas e incumplimientos de tu ficha</p>`. |
...
| `34` | `<button class="nav-link active" data-bs-toggle="tab" data-bs-target="#ta...` | Botón de acción interactivo para el usuario: `<button class="nav-link active" data-bs-toggle="tab" data-bs-target="#ta...`. |
| `35` | `<i class="fas fa-bell me-1"></i>Todas (<?= count($notificaciones) ?>)` | Instrucción de ejecución en el contexto del script: `<i class="fas fa-bell me-1"></i>Todas (<?= count($notificaciones) ?>)`. |
| `36` | `</button>` | Instrucción de ejecución en el contexto del script: `</button>`. |
...
| `39` | `<button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabIncump...` | Botón de acción interactivo para el usuario: `<button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabIncump...`. |
| `40` | `<i class="fas fa-triangle-exclamation me-1"></i>Incumplimientos (<?= cou...` | Instrucción de ejecución en el contexto del script: `<i class="fas fa-triangle-exclamation me-1"></i>Incumplimientos (<?= cou...`. |
| `41` | `</button>` | Instrucción de ejecución en el contexto del script: `</button>`. |
...
| `53` | `<i class="fas fa-bell-slash fa-2x mb-2 opacity-25 d-block"></i>` | Instrucción de ejecución en el contexto del script: `<i class="fas fa-bell-slash fa-2x mb-2 opacity-25 d-block"></i>`. |
...
| `59` | `$iconos = ['incumplimiento' => 'triangle-exclamation text-danger',` | Instrucción de ejecución en el contexto del script: `$iconos = ['incumplimiento' => 'triangle-exclamation text-danger',`. |
...
| `62` | `'info'           => 'circle-info text-primary'];` | Instrucción de ejecución en el contexto del script: `'info'           => 'circle-info text-primary'];`. |
...
| `71` | `<div class="fw-semibold small"><?= htmlspecialchars($n['titulo']) ?></div>` | Contenedor visual estructurado con Bootstrap/CSS: `<div class="fw-semibold small"><?= htmlspecialchars($n['titulo']) ?></div>`. |
| `72` | `<div class="text-muted small"><?= htmlspecialchars($n['mensaje']) ?></div>` | Contenedor visual estructurado con Bootstrap/CSS: `<div class="text-muted small"><?= htmlspecialchars($n['mensaje']) ?></div>`. |
| `73` | ``</div>`` | Cierre de contenedor visual `<div>`. |
| `74` | `<div class="text-muted" style="font-size:.75rem; white-space:nowrap;">` | Contenedor visual estructurado con Bootstrap/CSS: `<div class="text-muted" style="font-size:.75rem; white-space:nowrap;">`. |
...
| `79` | `<?php endforeach; ?>` | Cierre de ciclo iterativo foreach embebido en la plantilla HTML. |
...
| `92` | `<i class="fas fa-check-circle text-success fa-2x mb-2 d-block"></i>` | Instrucción de ejecución en el contexto del script: `<i class="fas fa-check-circle text-success fa-2x mb-2 d-block"></i>`. |
| `93` | `<span class="small">Sin incumplimientos registrados.</span>` | Instrucción de ejecución en el contexto del script: `<span class="small">Sin incumplimientos registrados.</span>`. |
...
| `97` | `<table class="table tabla-limpia align-middle mb-0">` | Tabla de datos para despliegue estructurado de información. |
| `98` | `<thead class="table-light">` | Definición de estructura semántica de cabecera o cuerpo de tabla. |
| `99` | ``<tr>`` | Fila contenedora de datos dentro de la tabla. |
| `100` | `<th>Fecha</th>` | Celda de encabezado de columna: `<th>Fecha</th>`. |
| `101` | `<th>Ficha</th>` | Celda de encabezado de columna: `<th>Ficha</th>`. |
| `102` | `<th>Módulo</th>` | Celda de encabezado de columna: `<th>Módulo</th>`. |
| `103` | `<th>Detalle</th>` | Celda de encabezado de columna: `<th>Detalle</th>`. |
| `104` | ``</tr>`` | Cierre de fila contenedora de tabla. |
| `105` | `</thead>` | Definición de estructura semántica de cabecera o cuerpo de tabla. |
| `106` | `<tbody>` | Definición de estructura semántica de cabecera o cuerpo de tabla. |
...
| `109` | `<td class="small text-muted"><?= date('d/m/Y H:i', strtotime($n['fecha']...` | Celda de contenido de tabla: `<td class="small text-muted"><?= date('d/m/Y H:i', strtotime($n['fecha']...`. |
| `110` | `<td><span class="badge bg-light text-dark border"><?= htmlspecialchars($...` | Celda de contenido de tabla: `<td><span class="badge bg-light text-dark border"><?= htmlspecialchars($...`. |
| `111` | `<td class="small"><?= htmlspecialchars($n['nombre_modulo'] ?? '—') ?></td>` | Celda de contenido de tabla: `<td class="small"><?= htmlspecialchars($n['nombre_modulo'] ?? '—') ?></td>`. |
| `112` | `<td class="small text-muted"><?= htmlspecialchars($n['mensaje']) ?></td>` | Celda de contenido de tabla: `<td class="small text-muted"><?= htmlspecialchars($n['mensaje']) ?></td>`. |
| `113` | ``</tr>`` | Cierre de fila contenedora de tabla. |
| `114` | `<?php endforeach; ?>` | Cierre de ciclo iterativo foreach embebido en la plantilla HTML. |
| `115` | `</tbody>` | Definición de estructura semántica de cabecera o cuerpo de tabla. |
| `116` | ``</table>`` | Cierre de tabla de datos. |
| `117` | ``</div>`` | Cierre de contenedor visual `<div>`. |
| `118` | `<?php endif; ?>` | Cierre de estructura condicional embebida en la plantilla HTML. |
...
| `125` | ``<style>`` | Bloque de estilos CSS personalizados para la interfaz. |
| `126` | `.rounded-top-0 { border-top-left-radius:0!important; border-top-right-ra...` | Instrucción de ejecución en el contexto del script: `.rounded-top-0 { border-top-left-radius:0!important; border-top-right-ra...`. |
...
| `129` | ``</style>`` | Cierre de bloque de estilos CSS. |
| `130` | `*(Línea en blanco)*` | Línea en blanco para organización visual y legibilidad. |
| `131` | `<?php require_once __DIR__ . '/../layouts/footer.php'; ?>` | Importación e inclusión obligatoria del archivo de dependencia requerido: `<?php require_once __DIR__ . '/../layouts/footer.php'; ?>`. |
